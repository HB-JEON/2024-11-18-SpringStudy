...
public interface CommentMapper {
	...
	// 수정
	@Update("UPDATE busanReply SET msg=#{msg} "
			+ "WHERE no=#{no}")
	...
	// 답글
	@Select("SELECT group_id, group_step FROM busanReply "
			+ "WHERE no=#{no}")
	public CommentVO commentParentInfoData(int no);
	
	@Update("UPDATE busanReply SET group_step=group_step+1 "
	...
	@Insert("INSERT INTO busanReply(no, cno, type, userid, username, sex, msg, group_id, group_step) "
			+ "VALUES((SELECT NVL(MAX(no)+1,1) FROM busanReply), #{cno}, #{type}, #{userid}, #{username}, "
			+ "#{sex}, #{msg}, #{group_id}, #{group_step})")
