...
// ---- request(model) 전송 불가능 _ok
// return "redirect: list.do"
// 리턴형 => void(다운로드), String
/*
    1. 매칭
        사용자 URI 매칭 => 메소드를 찾아 호출
        -------------
        @GetMapping(URI)
        @PostMapping
        ------------ + @RequestMapping
    2. 매개변수
        => request.getParameter 처리 후 매개변수에 전송
           --------------------
            => 사용자가 전송한 데이터는 String
            => 변환 후 받는 경우
...
public class RecipeController {
	// 스프링에 저장되는 객체를 가지고 온다. => 스프링에서 지역번수는 제어 불가능
	// 스프링은 전역 변수만 제어 가능 => @Autowired
	// router
